...
**Correction note (FINAL-GATE-R1):** this minimal, conditional correction fixes two material defects found while auditing the MASTER chain for release: (1) **wrong canonical slug / false unreachable claim.** EVIDENCE-LEDGER-R2B recorded the canonical repository as unreachable, using the hyphenated slug `tsamaan/OttoGuide-Proyecto-SIP-Grupo6-UADE`. That slug does not exist. The correct slug, `tsamaan/OttoGuide-Proyecto_SIP-Grupo6-UADE` (underscore), is confirmed reachable this checkpoint via read-only `git ls-remote`, with `main` and `review/orchestrator-unification` both matching the mirror exactly (`main=3a1f13574e4a27d9aff2bfd38b3659951e8cb264`, `review/orchestrator-unification=254cdddfa33477b794561560b397418870d777d9`) — the two repositories are aligned, not divergent. The two prose sentences that repeated the wrong-slug unreachable claim are corrected in place above (not silently deleted — see the inline notes at each). (2) **bounded manual motion vs. autonomous navigation.** The ledger's `movement` domain (below) correctly states the MASTER chain (`0d14de6`..`ff466a7`) introduced no locomotion code and that autonomous navigation is not validated anywhere — but it said nothing about a real, separate, pre-existing capability: a bounded/manual G1 motion wrapper (`tools/hil/robot_motion/ottoguide_g1_motion_v3.cpp`, using `unitree::robot::g1::LocoClient`), introduced by commits `ed3e815`/`0510075` (both confirmed ancestors of `review/orchestrator-unification`'s head `254cddd`, both predating the MASTER chain's own root parent — i.e. `introduced_by_master_chain: false`) and physically validated in a prior HIL session (`ROBOT_R5F_R3C_VISIBLE_MOTION_LADDER.md`: `yaw-l1` and `linear-l2` profiles confirmed visible/stable by the operator, no anomaly, session closed cleanly). A short subsection is added under "Movement / autonomous navigation" below to state this explicitly, so "MASTER introduced no locomotion" and "autonomous navigation not validated" are never misread as "no motion capability exists or has ever been validated in this project." No new domain was created and the existing 59 domains were not restructured. This checkpoint built no runtime or production package — only an external evidence package (`MASTER_OFFLINE_R1_RELEASE_CANDIDATE_COMPLETE.zip`, `MASTER_OFFLINE_R1_FINAL_GATE_AND_STAGING` run root). See `LEDGER_RELEASE_FINDINGS.json` in that run root for the full findings trail.
...
- **evidence_level:** COMMIT_EXACT
- **supporting_evidence_levels:** HASH_VERIFIED_ARTIFACT
- **source_paths:** `OttoGuide-Mapping-Workspace/_ODOM_R5_LOCAL_COMMIT_DECISION_WITH_RUFF_GAP_NO_PUBLISH/run_20260707T203329Z/ODOM_R5_COMMIT_LOG.txt` (commit `92a8bc45a7a8d7557bcdca9ae5684692016168a7`, 12 files, 1021 insertions); this checkpoint's own `git cat-file -t`, `git merge-base --is-ancestor`, `git rev-list --count`, and `git rev-parse <rev>:<path>` against `C:\OG\master-offline-r1-local-r1\repo` (tracking `origin=LucasCap12/OttoGuide-Proyecto_SIP-Grupo6-G1-EDU.git`), cross-verified live via `git ls-remote` against both the canonical (`tsamaan/OttoGuide-Proyecto_SIP-Grupo6-UADE` — note the underscore; a prior checkpoint tried a hyphenated variant of this slug and wrongly recorded it as unreachable, corrected in FINAL-GATE-R1 below) and mirror repositories
...
### Bounded manual motion wrapper (pre-existing, not introduced by the MASTER chain)

*(Added in FINAL-GATE-R1 — see the correction note near the top of this document. Not a new domain; this is a clarifying note attached to the Movement section above, so the `NOT_IMPLEMENTED`/`NOT_VALIDATED` classification above is never misread as "no motion capability of any kind exists or has been validated in this project.")*

- **bounded_robot_motion_wrapper.implementation:** present_in_review_baseline
- **bounded_robot_motion_wrapper.physical_validation:** bounded_manual_profiles_validated
- **bounded_robot_motion_wrapper.introduced_by_master_chain:** false
- **physical_autonomous_navigation.validation:** NOT_VALIDATED
- **source_paths:** `codigo ottoguide/tools/hil/robot_motion/ottoguide_g1_motion_v3.cpp` (introduced by `ed3e815`, touched by `0510075` — both confirmed ancestors of `review/orchestrator-unification`'s head `254cddd` via `git merge-base --is-ancestor`, both predating the MASTER chain's own root parent); `docs/Operaciones_HIL/ROBOT_R5F_R2_CXX_MINIMAL_MOTION_WRAPPER.md`, `docs/Operaciones_HIL/ROBOT_R5F_R3C_VISIBLE_MOTION_LADDER.md`
- **source_hashes:** blob for `ottoguide_g1_motion_v3.cpp` is byte-identical (`2081344...`) at `254cddd` and at this ledger's own HEAD
- **detail:** ROBOT-R5F-R3C physically validated two bounded, hardcoded, operator-gated motion profiles on the real G1: `yaw-l1` (~5.7° turn) and `linear-l2` (~12cm displacement), both confirmed visible and stable by the operator, zero anomalies, every profile closed with `StopMove()`, `Damp()` isolated to its own explicit `passive-damp` mode and never called implicitly. This is real, physically-validated, **bounded and manually-gated** motion — categorically distinct from, and not any evidence toward, autonomous navigation.
- **limitations:** this capability predates and is entirely independent of the MASTER chain audited in this ledger (`0d14de6`..`ff466a7`) — it is neither introduced nor modified by any MASTER commit. It does not constitute autonomous navigation, closed-loop control, obstacle avoidance, or any unsupervised movement; every profile requires an explicit, separate operator-issued CLI invocation and confirmation.
