...
| 2026-06-20 | Cierre de Test Post-Write y Semántica | Se implementaron los tests de fallo post-write real, la equivalencia lógica del reporte antes/después del rollback, el éxito condicionado a limpieza completa de temporales/backups/lock (residuos=0), la clasificación estricta de errores (CONTRACT/OPERATIONAL/ROLLBACK), y exit code 1 para fallos de filesystem/limpieza; el estado físico sigue siendo NO_GO. |
| 2026-06-20 | Corrección de Orden de Lock | Se implementaron la conservación del lock durante el rollback, liberación al final de transacciones exitosas, ownership del lock mediante transaction_id, PID y timestamp, rollback_performed=false para pre-staging, y tests unitarios y de CLI E2E integrados; el estado físico sigue siendo NO_GO. |
