...
El sellador adquiere un bloqueo exclusivo (`.gtseal.lock`) escribiendo un JSON con el `transaction_id` de la transacción, el PID del proceso y un timestamp UTC para validar la propiedad (ownership) del lock; ningún proceso eliminará un lock ajeno. Luego, crea copias de seguridad de la evidencia previa si existía y escribe archivos temporales de staging. Realiza el reemplazo ordenado de los archivos en disco usando `os.replace`. Finalmente realiza una validación posterior real. El éxito del sellado está estrictamente condicionado a la limpieza completa (cero temporales y cero backups), liberando el lock propio como última operación. Si ocurre un fallo en el éxito o durante la limpieza, se ejecuta rollback manteniendo el lock adquirido; el lock propio se conserva durante toda la restauración y validación del rollback, liberándose únicamente después de verificar que el rollback fue exitoso. Si la restauración falla o el lock propio no puede liberarse, se genera un error de categoría ROLLBACK (exit code 1). La bandera `rollback_performed` es `true` únicamente si se modificaron targets físicos en disco (staging iniciado), permaneciendo `false` para fallos pre-staging (como contrato o JSON fuente inválidos, pre-existencia de evidencia sin `--force` o locks ajenos ya existentes). Los fallos se clasifican en CONTRACT (exit code 3), OPERATIONAL (exit code 1) y ROLLBACK (exit code 1). Todo este comportamiento y salidas CLI fueron validados mediante tests unitarios y de integración end-to-end directos. El estado físico de la sesión continúa en NO-GO.
