...
- **MVP-ODOM-TF-R1A** (offline, Nivel B): hardening fail-closed del gate R1 (validacion de tipo del contrato y de estructura de candidatos, coherencia candidato/contrato para covarianza/IMU/evidencia dinamica, prohibicion de secuencias de canal mezcladas, orden temporal, y correccion de fiabilidad IMU en el adapter). Resultado sin cambios: `NOT_READY` con los mismos 11 blockers. Estados `/odom`, TF y Nav2 permanecen en `NOT_STARTED`; `physical validation = PENDING`. El `NEXT_ACTION` de unificacion permanece sin cambios.
- **MVP-ODOM-TF-R1B** (offline, Nivel B): cierra las rutas false-ready restantes y declara la serie R1 como un limite **no publicable**. Ningun conjunto de booleanos de contrato o candidato puede autorizar publicacion: `odom_publication_ready`, `odom_to_base_link_tf_ready` y `nav2_ready` son invariantes en `false` y `physical_validation_required` en `true`, con `publication_capability = WITHHELD_BY_R1_BOUNDARY`. Endurece la entrada: `isinstance(OdometryCandidate)` estricto + validacion de estructura completa (politicas, cuaternion de norma no nula, rpy, tipos de warnings/errors), adapter fail-closed ante entrada no-mapping, y semantica de strings del contrato (whitespace/allow-list/contradiccion). Covarianza y evidencia dinamica siempre contradicen en R1 (no hay modelo de covarianza ni objeto tipado de evidencia dinamica con ground truth). Resultado sin cambios: `NOT_READY` con los mismos 11 blockers. R2 debera introducir modelos de evidencia versionados. El `NEXT_ACTION` de unificacion permanece sin cambios.
- **MVP-ODOM-TF-R1C** (offline, Nivel B): cierra las rutas residuales donde una secuencia mixta valida/invalida, un iterable defectuoso o un Mapping defectuoso podian lanzar excepcion en lugar de un reporte fail-closed. Los `channels` del reporte se derivan solo de candidatos validos (sin ordenar `None` contra `str`; secuencia totalmente invalida -> `channels=()`), la materializacion de candidatos captura `except Exception` (nunca `BaseException`), el adapter exige un `collections.abc.Mapping` estricto con extraccion protegida fail-closed, y se rechaza `bool` en todo componente numerico dentro del gate de readiness (posicion/velocidad/rpy/cuaternion/yaw_speed). El limite no publicable de R1 permanece sin cambios (`publication_capability = WITHHELD_BY_R1_BOUNDARY`); resultado sin cambios: `NOT_READY` con los mismos 11 blockers. El `NEXT_ACTION` de unificacion permanece sin cambios.
- **MVP-ODOM-TF-R1D** (offline, Nivel B): cierra las rutas residuales de normalizacion de valores dentro de un `Mapping` bien formado que R1C no cubria -- `imu_quaternion`/`imu_rpy` ahora son campos requeridos y se validan (forma, finitud, norma no nula en cuaternion) antes de cualquier `tuple()`, los timestamps (`stamp_sec`, `stamp_nanosec`, `receipt_wall_utc_ns`) se validan en vez de defaultear silenciosamente, y `bool` se rechaza como numero tambien en el adapter (`validation.py`), no solo en el gate de readiness. Limite no publicable y resultado de fixtures reales sin cambios (`NOT_READY`, mismos 11 blockers, `publication_capability = WITHHELD_BY_R1_BOUNDARY`). El `NEXT_ACTION` de unificacion permanece sin cambios.
- **MVP-ODOM-TF-R1D-R1** (offline, Nivel B): remedia dos defectos reproducibles que una auditoria independiente pre-push de R1D encontro (`R1D_PRE_PUSH_AUDIT = NO_GO`) sin tocar el commit R1D. Normaliza los campos de timestamp de todo candidato invalido (`receipt_monotonic_ns`, `message_stamp_sec`, `message_stamp_nanosec`, `receipt_wall_utc_ns`) para que nunca conserven un `bool` ni un objeto arbitrario, quedando siempre serializables a JSON; y aisla los fallos de gyro/acelerometro auxiliares mediante un unico clasificador fail-closed (`_classify_imu_vector`/`_imu_reliability_and_warning`) que ya nunca invalida el candidato completo, solo degrada la fiabilidad de ese sensor. Elimina ademas el `except Exception` de ambito completo que R1D habia introducido como red de seguridad, en favor de helpers acotados y especificos por operacion. Limite no publicable y resultado de fixtures reales sin cambios (`NOT_READY`, mismos 11 blockers, `publication_capability = WITHHELD_BY_R1_BOUNDARY`). Su propia auditoria pre-push posterior encontro que `source_channel` habia quedado fuera de la normalizacion (`R1D_R1_PRE_PUSH_AUDIT = NO_GO`); ver R1D-R2. El `NEXT_ACTION` de unificacion permanece sin cambios.
- **MVP-ODOM-TF-R1D-R2** (offline, Nivel B): cierra los cuatro defectos que la auditoria pre-push de R1D-R1 encontro sobre `source_channel` no confiable -- candidato invalido no serializable, excepcion propagada desde `__eq__` u `__str__` hostiles, y canal no hashable rompiendo `validate_candidate_sequence()`. `to_odometry_candidate` ahora exige `type(channel) is str` antes de cualquier comparacion de membership (rechazando tambien subclases hostiles de `str` sin tocar su `__eq__`) con un mensaje de error constante y acotado; `_invalid()` normaliza cualquier canal no permitido o de tipo incorrecto al centinela fijo `"<invalid>"`; y `validate_candidate_sequence()` ya no confia en que todo candidato provenga del adapter, verificando `type(source_channel) is str` antes de usarlo como clave. Con esto, la garantia de "todo candidato invalido es canonico y serializable a JSON" que R1D-R1 declaraba queda cerrada de forma completa. Limite no publicable y resultado de fixtures reales sin cambios (`NOT_READY`, mismos 11 blockers, `publication_capability = WITHHELD_BY_R1_BOUNDARY`). El `NEXT_ACTION` de unificacion permanece sin cambios.
- **MVP-ODOM-TF-R1D-R3** (offline, Nivel B): cierra tres defectos que una auditoria independiente de GitHub sobre el commit R1D-R2 ya publicado en `mirror-lucas` encontro (`R1D_R2_GITHUB_INDEPENDENT_AUDIT = NO_GO_FOR_CANONICAL`), sin reinterpretar el `GO` de la propia auditoria pre-push de R1D-R2. Un nuevo helper puro compartido, `normalize_finite_vector()`, es ahora el unico lugar donde se toca el valor bruto de `position`/`velocity`/`imu_quaternion`/`imu_rpy`: valida y canoniza en una sola pasada, cerrando la reiteracion que antes rompia una secuencia cuyo `__iter__` fallaba recien en su segunda llamada. Los helpers numericos compartidos (`is_finite_number`, `is_positive_int`, `is_nonnegative_int`) ahora exigen `type(x) is int`/`float` exacto en vez de `isinstance()`, rechazando cualquier subclase hostil de `int`/`float` antes de invocar sus propios dunders de comparacion/conversion. El chequeo estructural del gate de readiness (`_is_structurally_valid_candidate`) reutiliza estos mismos helpers endurecidos en vez de duplicar su propia logica, exige `type(c) is OdometryCandidate` exacto (rechazando tambien una subclase autentica, no solo un duck-type) y `type(source_channel) is str` antes del membership test; ademas, el bucle de validacion estructural agrega una frontera estrecha por candidato (`except Exception`, nunca `BaseException`) para que un error de programacion inesperado degrade solo ese candidato en vez de propagarse fuera de toda la evaluacion. Limite no publicable y resultado de fixtures reales sin cambios (`NOT_READY`, mismos 11 blockers, `publication_capability = WITHHELD_BY_R1_BOUNDARY`). El `NEXT_ACTION` de unificacion permanece sin cambios. Una auditoria independiente pre-push posterior de este commit encontro tres defectos adicionales (`R1D_R3_PRE_PUSH_AUDIT = MVP_ODOM_TF_R1D_R3_PRE_PUSH_AUDIT_NO_GO`); ver R1D-R4.
- **MVP-ODOM-TF-R1D-R4** (offline, Nivel B): cierra los tres defectos que la auditoria pre-push de R1D-R3 encontro mas tres gaps adyacentes del mismo dominio detectados durante esa misma auditoria, sin reinterpretar el `NO_GO` de R1D-R3. Un nuevo helper puro, `normalize_finite_number()`, es ahora la unica definicion de "numero finito" que reutilizan tanto `is_finite_number` como `normalize_finite_vector`: exige `type(value) is int`/`float` y captura `OverflowError` (ademas de `TypeError`/`ValueError`) al convertir a `float`, cerrando el `OverflowError` real que `math.isfinite()` lanzaba para un `int` de precision arbitraria (`10**10000`) en `yaw_speed` y en cada componente vectorial. Dos constantes nuevas, `MAX_SIGNED_64` y `MAX_MESSAGE_NANOSEC`, acotan los helpers enteros (ahora `is_bounded_positive_int`/`is_bounded_nonnegative_int`) -- un limite interno de almacenamiento/serializacion unicamente, que no resuelve el mapeo a ROS Time ni la autoridad del timestamp -- cerrando el `ValueError` de `json.dumps` que un timestamp entero sin cota producia. El chequeo estructural de readiness ahora exige `type(...) is str` antes de comparar `timestamp_policy`/`frame_id`/`covariance_policy` (igual que ya hacia con `source_channel`), valida `receipt_wall_utc_ns` estructuralmente por primera vez, y `_validate_contract()` exige `type(contract) is OdomTfEvidenceContract` y `type(value) is bool`/`str` antes de cualquier `strip()`/comparacion, con una frontera estrecha adicional en `assess_odom_tf_readiness()` alrededor de esa validacion. El test contractual `test_malformed_candidate_never_raises_contract` ahora afirma el resultado fail-closed completo, no solo la ausencia de excepcion. Limite no publicable y resultado de fixtures reales sin cambios (`NOT_READY`, mismos 11 blockers, `publication_capability = WITHHELD_BY_R1_BOUNDARY`). El `NEXT_ACTION` de unificacion permanece sin cambios.
- **MVP-ODOM-TF-R2-P2** (offline, Nivel B): introduce el contrato versionado `2.2.0-p2` de semantica de frames y covarianza, separando nombres configurados, evidencia fisica preservada, replay offline, simulacion y estructura. El material de mapping se incorpora read-only como provenance sin promover configuraciones historicas a verdad fisica. Las estadisticas P1/P1A quedan tipadas como evidencia `PARTIAL_QUANTIFIED` en unidades de fuente; no existe conversion SI ni matriz ROS publicable. El legado queda endurecido: `activation_allowed()` es un wrapper deprecado que nunca autoriza publicacion, y 0.50/0.10/999 quedan marcados `LEGACY_PLACEHOLDER_NOT_EVIDENCE`. Resultado contractual: P2/replay/simulacion estructuralmente listos; publicacion fisica y simulada, TF y Nav2 permanecen bloqueados/diferidos. No hubo ROS, DDS, mapping, simulador, robot, SSH ni push. El `NEXT_ACTION` de unificacion permanece sin cambios.
- **MVP-ODOM-TF-R2-P2A** (offline, Nivel B): audita adversarialmente P2 y endurece los limites fail-closed de tipos, numeros, contenedores, P1A, provenance, mapping, dominios de covarianza, readiness, replay y politica de simulacion bajo el schema `2.2.1-p2a`. La evidencia seleccionada de mapping queda vinculada por un manifest versionado que verifica tanto los hashes de fuentes como sus manifests de origen. El artefacto P1A preservado se acepta bajo cuarentena por su hash exacto aunque conserva historicamente un canal preferido no nulo; cualquier variante o entrada sintetica con preferencia no nula se rechaza. La autoridad remota queda explicita: `tsamaan/OttoGuide-Proyecto_SIP-Grupo6-UADE` es canonico, `LucasCap12/OttoGuide-Proyecto_SIP-Grupo6-G1-EDU` es staging y el feature local no se atribuye a ninguno. No hubo escritura remota, ROS, DDS, mapping, simulador, robot ni SSH. Replay y politica de simulacion quedan contractualmente listos, pero adapter, modelo y ejecucion de simulacion permanecen en `false`; publicacion fisica, `/odom`, TF y Nav2 permanecen bloqueados. El `NEXT_ACTION` de unificacion permanece sin cambios.
