...
`FINAL_RELEASE_TREE` describe el contenido estable del árbol. No describe qué ref remoto canónico apunta a él en un instante determinado. El estado de `review` y `main` se resuelve directamente desde GitHub antes de cualquier operación; el snapshot final no depende de un segundo repositorio.
...
ramas laterales/personales, feature/* y audit/*
...
El contrato de publicación vigente es canónico y autocontenido:

1. resolver los refs canónicos frescos;
2. preservar `review/orchestrator-unification` como historia integrada inmutable;
3. auditar el árbol curado de entrega;
4. crear un commit raíz `R` con `parents(R) = []`;
5. reemplazar `canonical/main` sólo mediante lease/CAS, con autorización fresca, y verificar sus invariantes.

Este procedimiento es política durable, no un `NEXT_ACTION` de producto. No requiere publicar, consultar ni alinear un segundo repositorio.
...
La única excepción al fast-forward es el reemplazo curado de `canonical/main` por un root final, protegido por lease o compare-and-swap equivalente y conforme a `CIERRE_FINAL_MVP.md`.
...
- su tree es la entrega canónica final auditada;
- `canonical/main` es autocontenida y no depende de un segundo repositorio;
