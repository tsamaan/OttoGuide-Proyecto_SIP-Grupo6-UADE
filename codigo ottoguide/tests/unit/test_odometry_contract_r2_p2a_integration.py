...
def test_unification_state_has_final_release_boundaries():
    document = json.loads(
    ...
    assert document["kind"] == "CURRENT_FINAL_RELEASE_TREE_STATE"
    assert document["branch_reconciliation"] == "CLOSED"
    assert document["productive_development"] == "FROZEN"
    assert document["repositories"] == {
        "canonical_authority": "tsamaan/OttoGuide-Proyecto_SIP-Grupo6-UADE"
    }
    assert document["branch_roles"]["integration_history"] == (
        "review/orchestrator-unification"
    )
    assert document["branch_roles"]["final_delivery"] == "main"
    release = document["main_release"]
    ...
    assert document["current_external_mirror_dependency"] == "NONE"
    assert document["current_publication_model_requires_external_mirror"] is False
    assert "mirror_staging" not in document["repositories"]
    assert "publication_gates" not in document
