...
def test_unification_state_represents_final_canonical_release_model():
    repo_root = Path(__file__).resolve().parents[3]
    ...
    assert state["kind"] == "CURRENT_FINAL_RELEASE_TREE_STATE"
    assert state["program_objective"] == "OTTOGUIDE_FINAL_PROJECT_CLOSURE"
    assert state["branch_reconciliation"] == "CLOSED"
    assert state["productive_development"] == "FROZEN"
    assert state["next_action"] == "NO_FURTHER_PRODUCTIVE_DEVELOPMENT"
    assert state["repositories"]["canonical_authority"] == (
        "tsamaan/OttoGuide-Proyecto_SIP-Grupo6-UADE"
    )
    assert state["branch_roles"]["integration_history"] == (
        "review/orchestrator-unification"
    )
    assert state["branch_roles"]["final_delivery"] == "main"
    release = state["main_release"]
    assert release["model"] == "SINGLE_ROOT_FINAL_RELEASE"
    assert release["required_parent_count"] == 0
    assert release["required_commit_count"] == 1
    assert release["merge_or_rebase_with_review"] == "PROHIBITED"
    assert release["blind_force"] == "PROHIBITED"
    assert state["current_external_mirror_dependency"] == "NONE"
    assert state["current_publication_model_requires_external_mirror"] is False
    assert state["historical_provenance_external_reference_allowed"] is True
    assert "mirror_staging" not in state["repositories"]
    assert "publication_gates" not in state
    assert "BYTE_PRESERVED_U_SERIES_PREDECESSORS_SUPERSEDED_AS_ACTIVE_AUTHORITY" in handoff
    for value in (
    ...
    assert "MIRROR_STAGING =" not in handoff
    assert "mirror review fast-forward" not in handoff
    assert "P2C" not in state_path.read_text(encoding="utf-8")
