...
@router.post(
    "/tour/start-qr-fsm",
    status_code=status.HTTP_202_ACCEPTED,
    response_model=StartQRFSMTourResponse,
    summary="Iniciar recorrido reactivo por QR sin waypoints digitales",
)
async def endpoint_start_qr_fsm_tour(
    request: Request,
    payload: StartQRFSMTourRequest,
    background_tasks: BackgroundTasks,
    orchestrator=Depends(_get_orchestrator),
) -> StartQRFSMTourResponse:
    """
    @TASK: Despachar FSM QR reactiva como recorrido principal.
    @CONTEXT: No recibe waypoints; el movimiento y los estados se gobiernan por QR.
    @SECURITY: TransitionNotAllowed/estado no idle se registran y no bloquean la respuesta HTTP.
    """
    readiness_errors = await _resolve_readiness_errors(request, orchestrator)
    if readiness_errors:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail={
                "message": "Sistema no listo para iniciar recorrido QR FSM.",
                "errors": readiness_errors,
            },
        )
    ...
    async def _dispatch():
        try:
            await orchestrator.dispatch_qr_fsm_tour(tour_id=payload.tour_id)
        except TransitionNotAllowed as exc:
            LOGGER.error("[API] dispatch_qr_fsm_tour rechazado: %s", exc)
        except Exception as exc:
            LOGGER.error("[API] Excepcion en dispatch_qr_fsm_tour: %s", exc)

    background_tasks.add_task(_dispatch)

    LOGGER.info("[API] POST /tour/start-qr-fsm aceptado. tour_id=%s", payload.tour_id)
    return StartQRFSMTourResponse(
        accepted=True,
        detail=f"Recorrido QR FSM '{payload.tour_id}' aceptado.",
        tour_id=payload.tour_id,
    )
