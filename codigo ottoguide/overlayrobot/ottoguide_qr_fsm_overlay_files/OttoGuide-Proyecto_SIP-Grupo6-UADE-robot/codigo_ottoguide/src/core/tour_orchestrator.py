...
class TourOrchestrator(StateMachine):
    ...
    async def dispatch_qr_fsm_tour(self, *, tour_id: str = "qr-fsm-tour-001") -> None:
        ...
        @OUTPUT: Estado NAVIGATING y tarea _qr_fsm_loop activa como _nav_task.
    ...
    def _build_qr_fsm_controller(self):
        ...
        registry = QRStationRegistry(project_root / settings.QR_FSM_CONFIG_PATH)
        detector = QRDetector(
            camera_index=settings.QR_FSM_CAMERA_INDEX,
            stable_frames=settings.QR_FSM_STABLE_FRAMES,
        )
        audio_player = LocalAudioPlayer(
            project_root=project_root,
            wav_player_cmd=settings.QR_FSM_WAV_PLAYER_CMD,
            mp3_player_cmd=settings.QR_FSM_MP3_PLAYER_CMD,
        )
        motion = QRMotionDriver(
            hardware=self._hardware_api,
            forward_speed_mps=settings.QR_FSM_FORWARD_SPEED_MPS,
            approach_speed_mps=settings.QR_FSM_APPROACH_SPEED_MPS,
            turn_angular_z=settings.QR_FSM_TURN_ANGULAR_Z,
            move_tick_ms=settings.QR_FSM_MOVE_TICK_MS,
        )
        ...
        return OttoQRAsyncFSM(
            ...
            motion=motion,
            llm=llm,
            pre_stop_delay_s=settings.QR_FSM_PRE_STOP_DELAY_S,
            cooldown_s=settings.QR_FSM_COOLDOWN_S,
            max_turn_duration_s=settings.QR_FSM_MAX_TURN_DURATION_S,
            prevent_repeated_qr=settings.QR_FSM_PREVENT_REPEATED_QR,
        )
    ...
    async def _qr_fsm_loop(self) -> None:
        ...
        """
        controller = self._build_qr_fsm_controller()
