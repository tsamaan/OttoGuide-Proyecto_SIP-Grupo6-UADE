...
**Fecha**: 2026-06-20
**Fase**: Fase 2C + corrección acotada de parámetros namespaced — control local closed-loop exclusivamente simulado (`controller_server` + local costmap). La causa raíz original (carga incorrecta de parámetros anidados de plugin) está **resuelta**: `controller_server` alcanza `ACTIVE` y `FollowPath` completa `SUCCEEDED`. Sigue **PARTIAL** porque apareció un problema runtime distinto y nuevo: el movimiento simulado y la parada tras cancelación no están confirmados. No incluye BT Navigator, behavior server, waypoint follower, Simple Commander ni Collision Monitor.
...
| `LOCAL_CONTROL_SANDBOX` | **NOT_READY (PARTIAL, causa raíz original resuelta)** | `controller_server` + `local_costmap` + simulador con integración cinemática y allowlist de velocidad (`cmd_vel_raw` único permitido) están implementados. La carga incorrecta de parámetros anidados de plugin (`FollowPath.plugin`, `FollowPath.critics`, `GridBased.tolerance`) está corregida mediante reescritura namespaced (`ParameterFile`/`RewrittenYaml` en el launch): `controller_server` ahora alcanza `active` de forma reproducible y `/offline_nav/follow_path` devuelve `SUCCEEDED`. No se declara `READY` porque el escenario de movimiento simulado observable (`simulated_distance_moved: 0.0`) y el de cancelación con parada confirmada (`stop_after_cancel: FAIL`) no pasan, por un problema runtime distinto y nuevo aún no investigado. Detalle completo en [OFFLINE_NAVIGATION_LOCAL_CONTROL_REPORT.md](OFFLINE_NAVIGATION_LOCAL_CONTROL_REPORT.md). |
...
- No declara `LOCAL_CONTROL_SANDBOX` como `READY`: aunque `controller_server` llega a `active` y `FollowPath` devuelve `SUCCEEDED`, el movimiento simulado observable y la parada confirmada tras cancelación no están validados.
...
- No declara que el problema de movimiento simulado/cancelación esté diagnosticado o resuelto; queda documentado como bloqueo nuevo pendiente de investigación.
...
- Tras esta corrección, apareció un problema runtime distinto y nuevo, no relacionado con la carga de parámetros: el escenario de movimiento simulado no detecta avance de pose y el de cancelación no confirma velocidad cero dentro de la ventana esperada. Documentado en [OFFLINE_NAVIGATION_LOCAL_CONTROL_REPORT.md](OFFLINE_NAVIGATION_LOCAL_CONTROL_REPORT.md), sin investigar aún su causa raíz.
...
Antes de declarar `LOCAL_CONTROL_SANDBOX = READY`, se necesita investigar por qué `FollowPath` completa `SUCCEEDED` sin que el simulador registre avance de pose, y por qué la cancelación no produce velocidad cero confirmada — ver [OFFLINE_NAVIGATION_LOCAL_CONTROL_REPORT.md](OFFLINE_NAVIGATION_LOCAL_CONTROL_REPORT.md). Esto no requiere instalar ni reinstalar paquetes; es un comportamiento a diagnosticar en la integración cinemática del simulador y/o en el timing del cliente de smoke test. Habilitar `ROS_RUNTIME_SANDBOX = READY` requiere además, con decisión explícita de aislamiento, `behavior_server`/`waypoint_follower`/`collision_monitor` y scripting con `nav2_simple_commander`, todo bajo el mismo aislamiento (`ROS_LOCALHOST_ONLY=1`, `ROS_DOMAIN_ID` dedicado, sin `/cmd_vel` real hacia hardware). Esa fase de misiones no se ejecutó en este incremento y queda fuera de su alcance.
