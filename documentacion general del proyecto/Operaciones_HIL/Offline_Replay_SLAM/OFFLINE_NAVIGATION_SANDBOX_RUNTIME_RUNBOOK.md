...
| Cualquier smoke test multi-escenario falla con `DERIVED_DOMAIN_ID_OUT_OF_RANGE` o `INVALID_DOMAIN_ID` antes de iniciar ningún nodo | `--base-domain-id` (o el ID derivado de un offset) cae fuera de `1..232`, o el valor suministrado no es un entero (`"abc"`, `"12.5"`, cadena vacía, solo espacios) | Comportamiento esperado: usar un `--base-domain-id` entero tal que `base` y `base + maximum_offset` del componente (`2` Behavior Server, `4` Collision Monitor, `1` BT Navigator) queden dentro de `1..232`. Desde Fase 2F.1, un `--base-domain-id` no entero devuelve `INVALID_DOMAIN_ID` con `exit=2` y JSON estructurado, nunca un traceback. |
